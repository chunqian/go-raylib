...
// GuiListViewEx function as declared in src/raygui.h:474
func GuiListViewEx(bounds Rectangle, text []string, count int32, focus *int32, scrollIndex *int32, active int32) int32 {
	cbounds, _ := bounds.PassValue()
	ctext, _ := unpackArgSString(text)
	ccount, _ := (C.int)(count), cgoAllocsUnknown
	cfocus, _ := (*C.int)(unsafe.Pointer(focus)), cgoAllocsUnknown
	cscrollIndex, _ := (*C.int)(unsafe.Pointer(scrollIndex)), cgoAllocsUnknown
	cactive, _ := (C.int)(active), cgoAllocsUnknown
	__ret := C.GuiListViewEx(cbounds, ctext, ccount, cfocus, cscrollIndex, cactive)
	packSString(text, ctext)
	__v := (int32)(__ret)
	return __v
}
